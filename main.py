        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(self.auth_response, dict):
                    self.access_token = self.auth_response.get(
                        "access_token"
    ...
    def get_usage_data(
        ...
        try:
            ...
            if data.get("status") == "PENDING":
                print(
                    f"✗ Data still pending after {max_retries} attempts. Try again later or increase retry limit."
                )
                return None
            elif (
                ...
                print(
                    f"✓ Response received with status: {data.get('status', 'unknown')}"
                )
                return data
...
def parse_arguments():
    ...
    parser.add_argument("-u", "--username", help="Utility account username/email")
    parser.add_argument("-p", "--password", help="Utility account password")
    parser.add_argument("-s", "--service-location", help="Service location number")
    parser.add_argument("-a", "--account-number", help="Account number")
    parser.add_argument(
        "-d",
        "--days",
        type=int,
...
def main():
    ...
    if not username or not password:
        ...
        print(
            "  1. Command line: python main.py -u your_email@gmail.com -p your_password"
        )
    ...
    if usage_data:
        ...
        if (
            ...
            and usage_data.get("status") == "COMPLETE"
        ):
            ...
            if total_records > 0:
                ...
                for utility in ["electric", "gas", "water"]:
                    if utility in stats:
                        ...
                        print(
                            f"  Total: {u_stats['total_usage']:.2f} {u_stats['unit']}"
                        )
                        print(
                            f"  Average: {u_stats['avg_usage']:.2f} {u_stats['unit']}"
                        )
