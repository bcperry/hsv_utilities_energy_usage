...
class EnergyDeltaStorage:
    ...
    def __init__(self, base_path: str = "./energy_data"):
        ...
        # Ensure full directory tree exists (including parents like /config)
    ...
    def save_fetch_metadata(
            ...
            [
                {
                    ...
                    "start_datetime": pd.to_datetime(
                        start_datetime, unit="ms", utc=True
                    ),
        ...
        write_deltalake(
            self.metadata_path, metadata, mode="append", schema_mode="merge"
        )
    ...
    def read_usage_data(
        ...
        """
        try:
            # Gracefully handle non-existent table on first run
            usage_dir = Path(self.usage_path)
            if not usage_dir.exists() or not (usage_dir / "_delta_log").exists():
                # Ensure base directories exist and return empty until first write
            ...
            # Return empty DataFrame on read issues to avoid noisy logs
    ...
    def get_stats(self) -> dict:
        ...
        try:
            # Overall usage stats (only count USAGE records, not COST)
            dt = DeltaTable(self.usage_path)
            df = dt.to_pandas(filters=[("data_type", "=", "USAGE")])
            ...
            # Stats by utility type (only USAGE records)
        ...
        try:
            ...
            stats["fetch_history"] = {
                ...
                "last_fetch": str(df_meta["fetch_timestamp"].max())
                if len(df_meta) > 0
                else None,
    ...
    def vacuum_table(self, retention_hours: int = 168):
        ...
        try:
            ...
            print("✓ Vacuumed old files from usage table")
        except Exception as e:
